...
#   SAMPLE    numero di documenti del campione generativo (default: 20)
#   SEED      seme che fissa il sorteggio del campione   (default: 42)
...
# Il campione su cui girano le configurazioni generative: le stesse cifre servono
# anche ai rilevatori tradizionali, perche' le righe isolate di
# tab:test-detection-confronto devono poggiare sul medesimo terreno delle righe AI.
SAMPLE="${SAMPLE:-20}"
SEED="${SEED:-42}"
...
if [ "${SKIP_DETECT:-0}" != "1" ]; then
  ...
  hr "§8.3.1  Gli stessi rilevatori sul solo campione  ->  righe isolate di tab:test-detection-confronto"
  # Stessa misura ristretta al campione generativo: serve (a) a riempire le righe
  # "solo pattern"/"solo NER" del confronto sul medesimo terreno delle righe AI e
  # (b) a verificare la rappresentativita' del campione contro il corpus intero.
  # shellcheck disable=SC2086
  $PY -m pii_detection.evaluation.run_presidio_baseline $gliner_flag $corpus_flag \
      --sample "$SAMPLE" --seed "$SEED" \
    | tee "$OUTDIR/02_presidio_sample${SAMPLE}.txt"
fi
...
if [ "${SKIP_AI:-0}" != "1" ]; then
  ...
  $PY -m pii_detection.evaluation.run_ai_benchmark $gliner_flag $limit_flag $corpus_flag \
      --models $MODELS --sample "$SAMPLE" --seed "$SEED" --per-category \
