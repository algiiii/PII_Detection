...
Il merge concettuale (§\ref{sub:b4-merge}) è realizzato da un unico componente (\texttt{MergeEngine}) che collassa i candidati delle sorgenti direttamente in \texttt{PIIMatch}, il tipo con cui le PII proseguono verso il registro. Non esistono tipi intermedi: la certezza di una rilevazione \emph{è} un insieme di campi del \texttt{PIIMatch} --- \texttt{confidence}, \texttt{confirmation\_level}, \texttt{sources}. L'unificazione incrocia i candidati per \textbf{sovrapposizione degli intervalli}: quelli che si sovrappongono e concordano sul tipo si fondono in una doppia (o tripla) conferma con confidenza rinforzata; quelli che si sovrappongono ma discordano restano \textbf{entrambi}, marcati come in conflitto, senza che il merge arbitri. Il candidato generativo entra per ultimo, nel doppio ruolo --- conferma o scoperta --- descritto in architettura. Il tutto secondo l'impostazione \textbf{recall-oriented}: nessun candidato viene scartato, al massimo segnalato come meno certo --- un conflitto è due \texttt{PIIMatch}, non un match perduto.
