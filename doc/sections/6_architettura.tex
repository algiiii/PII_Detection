...
\begin{figure}[htbp]
	...
	\end{tikzpicture}
	\caption[Dettaglio del blocco B4: pipeline ibrida di rilevamento PII]{Dettaglio del blocco B4: pipeline ibrida di rilevamento PII. I detector regex e NER sono forniti da Presidio (\texttt{AnalyzerEngine}) dietro il contratto \texttt{PIIDetector}; il merge resta proprietario. Il ramo tratteggiato in basso a destra --- il campionamento e la passata AI contestuale --- è \emph{previsto ma non ancora realizzato}: il punto di innesto esiste già nel merge, che accetta candidati di provenienza generativa e li classifica come tali, mentre il rilevatore che li produrrebbe non è al momento cablato nella pipeline (§\ref{sub:b4-ai-previsto}).}
...
Entrambe le tecniche sono esposte al resto del sistema attraverso un unico contratto, \texttt{PIIDetector} (nella forma \texttt{detect(testo)} $\rightarrow$ lista di candidati): l'orchestrazione dipende solo da questa forma, non dalla tecnologia sottostante, così una sorgente può essere sostituita senza toccare il resto del livello (cfr. §\ref{sec:valutazione-presidio}).
...
\begin{itemize}
	...
	\item \textbf{Scoperta dall'AI} --- individuata dalla passata generativa selettiva (campionata), assente dalle altre sorgenti. È l'unico dei quattro casi che il sistema, allo stato attuale, non può ancora produrre (§\ref{sub:b4-ai-previsto}).
\end{itemize}
...
\subsection{Il ramo generativo: un innesto predisposto}
\label{sub:b4-ai-previsto}

La passata AI rappresentata nella Figura~\ref{fig:b4-detection-pipeline} merita una precisazione di stato, perché è l'unica parte del blocco che descrive un'intenzione anziché un comportamento. L'idea è impiegare un modello generativo come \emph{secondo parere} su un campione dei documenti --- non su tutti, per la ragione di costo discussa in~\ref{sec:impatto-ambientale} --- così da misurare quanto l'AI aggiunga rispetto alle due sorgenti deterministiche e su quali categorie.

Ciò che è realizzato è l'\textbf{innesto}: il merge accetta una terza sorgente di candidati, li tratta come le altre nel confronto per sovrapposizione e conserva la loro provenienza generativa quando coprono una porzione di testo che nessun'altra sorgente ha toccato. Ciò che non è realizzato è il rilevatore che quei candidati dovrebbe produrre, né la politica di campionamento che decide quando invocarlo. La distinzione è rilevante nel leggere il capitolo: il vocabolario del merge è definito per quattro casi perché il quarto è \emph{strutturalmente} previsto, ma un'esecuzione odierna del sistema ne produce solo tre.
