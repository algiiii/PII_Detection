...
Come emerso dallo stato dell'arte (§\ref{sec:sintesi-gap}), Microsoft Presidio non risolve il problema di governance affrontato dal progetto; il suo \textit{motore di rilevamento}, però, copre gran parte del livello regex e NER che il sistema deve comunque realizzare. Occorre quindi decidere se realizzare tale livello riutilizzando il detection engine di Presidio oppure implementandolo ex novo. La decisione è affidata a una \textbf{valutazione empirica}: questa sezione ne fissa i termini, mentre l'esito --- l'adozione di Presidio, con GLiNER per il livello NER --- è riportato nel §\ref{sec:test-detection}.
