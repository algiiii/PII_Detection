...
\paragraph{PA-1 — Aggregazione delle PII frammentate.} Va deciso se e in che misura il sistema debba ricondurre frammenti dispersi (in uno o più documenti) a un'unica entità coerente, e con quali criteri verificabili (co-occorrenza, similarità semantica, identificatori condivisi). Requisito attualmente marcato \textsc{da valutare} (cfr.~§\ref{sub:aggregazione-pii}).

\paragraph{PA-2 — Costruzione di una knowledge base delle entità.} Va deciso se il sistema debba costruire, a partire dalle PII estratte, una rappresentazione strutturata delle entità (via NER) come base per il confronto col ROPA, e con quale livello di dettaglio. Requisito attualmente marcato \textsc{da valutare} (cfr.~§\ref{sub:knowledge-base}). Impatta direttamente su PA-1 e sullo \emph{scope} complessivo (cfr. rischio di \emph{scope creep}, §\ref{sub:rischio-scope-creep}).
...
\paragraph{PA-9 — Tecnologia di persistenza.} Il modello concettuale è \emph{technology-agnostic}; va scelta la tecnologia concreta tra base dati relazionale (es. SQLite, con \emph{join} nativi) e persistenza su file semi-strutturati (JSON/JSONL, più leggera ma senza garanzie relazionali). La decisione è deliberatamente rimandata a valle della chiusura del modello dati (in particolare di PA-6, PA-7, PA-8) (cfr.~§\ref{sec:implementazione-persistenza-tecnologia}).
...
\begin{itemize}
	\item \textbf{TB-1} — Stato dell'arte: sezione «Definizione e inquadramento del problema» da riempire senza ripetere il contesto; definizione del problema tecnico dell'astrazione ancora allo stato di appunto (cfr.~§\ref{sec:definizione}).
	\item \textbf{TB-2} — Stato dell'arte: la sezione sul \emph{compliance checking} automatizzato è forse da ricollocare (cfr.~§\ref{sec:sintesi-gap} e dintorni).
	\item \textbf{TB-3} — Architettura: confronto strutturato e scelta del modello NER (tabella dei tre approcci di PA-11) da inserire nel Capitolo~\ref{cap:architettura}.
	\item \textbf{TB-4} — Architettura: diagramma ER in TikZ del modello di persistenza (\texttt{PIIInstance}, \texttt{PIIChange}, \texttt{Document}, \texttt{Scan}, \texttt{ProcessingActivity}), da produrre dopo la chiusura di PA-6/7/8 (cfr.~§\ref{sub:persistenza-modello-dati}).
	\item \textbf{TB-5} — Implementazione: roadmap implementativa e di validazione (baseline zero-shot GLiNER, integrazione modello pre-fine-tunato, eventuale esplorazione di fine-tuning) nel Capitolo~\ref{cap:implementazione}.
	\item \textbf{TB-6} — Studi: sezione sull'impatto ambientale dell'AI da completare con i numeri dai paper di confronto e la conclusione (cfr.~§\ref{cap:studi}).
	\item \textbf{TB-7} — Studi: precisazione sul costo computazionale, distinguendo l'inferenza LLM ripetuta (esclusa per l'analisi massiva) dal fine-tuning \emph{una tantum} di un encoder locale (cfr.~§\ref{sec:considerazioni-training}).
\end{itemize}
